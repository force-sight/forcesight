...
from transformers import T5Model,T5EncoderModel, T5Tokenizer, BertModel, BertConfig, BertTokenizer, AutoTokenizer
...
def create_file_index(folder):
    ...
    if len(os.listdir(folder)) == 0:
        ...
        file_index = max(int(name.split('_')[-1].split('.')[0]) for name in os.listdir(folder)) + 1
    return file_index
...
def load_model(config, checkpoint_path=None):
    ...
    if config.IMAGE_MODEL.split('-')[0] == 'vit':
        ...
        if config.USE_RGBD:
            image_model.model.patch_embed.proj.requires_grad = True # train the patch embedding layer
        ...
        if config.FREEZE_IMAGE_MODEL:
            for param in clip_model.visual.parameters():
                param.requires_grad = False
        if config.USE_RGBD:
            clip_model.model.visual.conv1.requires_grad = True # train the patch embedding layer
    ...
    if config.MULTIMODAL_HEAD == 'concat-linear-attn-mlp':
        ...
        model = ConditionedCLIP(image_model, text_model='clip', config=config, hidden_dim=256) # modified cfg repo to use openai ViT-L/14 model
    elif config.MULTIMODAL_HEAD == 'classifier-free-guidance' and config.IMAGE_MODEL.split('-')[0] == 'dinov2':
        model = ConditionedDinov2(image_model, text_model='t5', config=config, hidden_dim=256)
...
def load_grip_force_model():
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    device = torch.device('cpu') # to avoid multiprocessing issues since grip_force_model is tiny and running in the data loader
...
def run_grip_force_model(grip_force_model, pos_dict, curr_left_fingertip, curr_right_fingertip):
    ...
    with torch.no_grad():
        curr_left_fingertip = torch.tensor(curr_left_fingertip).float().unsqueeze(0)
        curr_right_fingertip = torch.tensor(curr_right_fingertip).float().unsqueeze(0)
        gripper_pos = torch.tensor(pos_dict['gripper']).float().unsqueeze(0).unsqueeze(0)
        gripper_effort = torch.tensor(pos_dict['gripper_effort']).float().unsqueeze(0).unsqueeze(0)

        fingertip_dist = torch.norm(curr_left_fingertip - curr_right_fingertip, dim=1).float().unsqueeze(0)
...
def copy_folder_contents(src_folder, dest_folder):
    ...
    for filename in os.listdir(src_folder):
        ...
        if os.path.isfile(file_path):
            ...
            shutil.copytree(file_path, os.path.join(dest_folder, filename))  # copy directories
